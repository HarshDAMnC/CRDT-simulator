...
1. **Start the Engine**: Right click `web/index.html` via VSCode and select **"Open with Live Server"** (or just open it natively inside any Firefox/Chrome browser).
2. **Setup Multiplayer**: Duplicate your browser tab to natively isolate two separate windows and arrange them split-screen.
3. **Show Off CRDT Conflict Resolution**: 
    - Type rapidly in Tab 1, it will synchronize immediately to Tab 2.
    - Type on the exact same line simultaneously; you will notice none of the characters get scrambled.
4. **Highlight the Memory Model**: As you add/remove content natively, guide your interviewer to your Right-Sidebar. This displays the actual Fractional Index variables in real-time, mapping every individual keystroke directly onto its LSEQ algorithm tree value! 
